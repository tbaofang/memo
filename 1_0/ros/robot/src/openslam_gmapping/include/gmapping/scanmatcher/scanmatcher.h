...
#define LASER_MAXBEAMS 2048     // 最大激光点数,如果激光点超过2048,则需要增大该数
...
//此处的方法是likehood_field_range_finder_model方法，参考《概率机器人》手稿P130
inline double ScanMatcher::score(const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const{
	double s=0;
	const double * angle=m_laserAngles+m_initialBeamsSkip;  // 当前角度=激光雷达的角度+初始化光束
	OrientedPoint lp=p;     // p当前位置
	lp.x+=cos(p.theta)*m_laserPose.x-sin(p.theta)*m_laserPose.y;    // 计算激光雷达的位置x,y,theta
	lp.y+=sin(p.theta)*m_laserPose.x+cos(p.theta)*m_laserPose.y;
	lp.theta+=m_laserPose.theta;
	unsigned int skip=0;    // 设置一个阈值
	double freeDelta=map.getDelta()*m_freeCellRatio;    // m_freeCellRatio=sqrt(2.) map.getDelta()地图resolution
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++){
		skip++;
		skip=skip>m_likelihoodSkip?0:skip;
		if (skip||*r>m_usableRange||*r==0.0) continue;
		Point phit=lp;
		phit.x+=*r*cos(lp.theta+*angle);    // likehood_field_range_finder_mode line 5
		phit.y+=*r*sin(lp.theta+*angle);
		IntPoint iphit=map.world2map(phit); // iphit为珊格中激光点位置
		Point pfree=lp;                     // lp为激光雷达sensor的位置 
 		pfree.x+=(*r-map.getDelta()*freeDelta)*cos(lp.theta+*angle); // pfree每个激光点位置
		pfree.y+=(*r-map.getDelta()*freeDelta)*sin(lp.theta+*angle);
 		pfree=pfree-phit;                   // 欧式距离dist     
		IntPoint ipfree=map.world2map(pfree);
		bool found=false;
		Point bestMu(0.,0.);
		for (int xx=-m_kernelSize; xx<=m_kernelSize; xx++)      // kernelSize = 1
		for (int yy=-m_kernelSize; yy<=m_kernelSize; yy++){
			IntPoint pr=iphit+IntPoint(xx,yy);
			IntPoint pf=pr+ipfree;
			//AccessibilityState s=map.storage().cellState(pr);
	...
	} // end for (const double* r=readings
	return s;
}
