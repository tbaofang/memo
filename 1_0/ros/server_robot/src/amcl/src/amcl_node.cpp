        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("sl_enable", sl_enable_, false);
  ...
  global_loc_srv_ = nh_.advertiseService("global_localization", 
					 &AmclNode::globalLocalizationCallback,
                                         this);
  sl= nh_.advertise<std_msgs::Empty>("/self_localization", 2, true);
...
AmclNode::requestMap()
{
  ...
  ROS_INFO("Requesting map success");
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      if (tf_broadcast_ == true)
      {
        ...
        // amcl is totally start up 
        if ((sent_first_transform_) && (!amcl_running_) && (sl_enable_))
        {
          ROS_WARN("sending self localization signal!");
          std_msgs::Empty srv;
          sl.publish(srv);
          amcl_running_ = true;
        }
