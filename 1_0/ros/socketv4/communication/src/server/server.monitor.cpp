...
void ServerMonitor::runThread() {
    if(mode_ == "nav") {
        ros::Subscriber tf_static_sub = nh_.subscribe("/tf_static", 1, &ServerMonitor::tfStaticCb, this);
...
void ServerMonitor::tfStaticCb(const tf2_msgs::TFMessage::ConstPtr &msg) {
    tf_static_state_ = true;
}
